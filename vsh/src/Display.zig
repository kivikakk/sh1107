...
const TopDrawState = struct {
    ...
    pub fn start(display: *Display) TopDrawState {
        return .{
            .display = display,
            .left = @as(f32, @floatFromInt(DisplayBase.padding)),
            .top = @as(f32, @floatFromInt(DisplayBase.padding)),
        };
    }

    pub fn check(self: *TopDrawState, name: []const u8, value: bool) void {
        gk.gfx.draw.textOptions(name, self.display.base.fontbook, .{
            .x = self.left + @as(f32, @floatFromInt(DisplayBase.checkbox_size + DisplayBase.checkbox_text_gap)),
            .y = self.top,
        ...
        if (value) {
            // "bold"
            gk.gfx.draw.textOptions(name, self.display.base.fontbook, .{
                .x = self.left + @as(f32, @floatFromInt(DisplayBase.checkbox_size + DisplayBase.checkbox_text_gap)) + 0.5,
                .y = self.top,
                .alignment = .left_middle,
                .color = DisplayBase.white,
            });
        ...
        const y = self.top - @as(f32, @floatFromInt(DisplayBase.checkbox_up));
        ...
        self.top += @as(f32, @floatFromInt(DisplayBase.top_row_height));
    }
    ...
    pub fn row(self: *TopDrawState) void {
        self.left += @as(f32, @floatFromInt(DisplayBase.top_col_width));
        self.top = @as(f32, @floatFromInt(DisplayBase.padding));
    }
};
...
fn drawOLED(self: *Display, sh1107: *const SH1107) void {
    ...
    if (sh1107.power) {
        gk.gfx.draw.hollowRect(
            .{
                .x = @as(f32, @floatFromInt(DisplayBase.padding)),
                .y = @as(f32, @floatFromInt(DisplayBase.padding + DisplayBase.top_area)),
            },
        ...
        const x_scale = @as(f32, @floatFromInt(DisplayBase.display_scale)) * x_factor;
        const shift = if (sh1107.com_scan_dir == .Backwards) @as(f32, @floatFromInt(DisplayBase.i2c_width)) else 0;
        const display_scale = @as(f32, @floatFromInt(DisplayBase.display_scale));
        ...
        gk.gfx.draw.texScaleXYRegionAngle(
            self.img,
            .{
                .x = @as(f32, @floatFromInt(DisplayBase.padding + DisplayBase.border_width)) + @as(f32, @floatFromInt(DisplayBase.i2c_width)) * display_scale,
                .y = @as(f32, @floatFromInt(DisplayBase.padding + DisplayBase.border_width + DisplayBase.top_area)) + shift * display_scale,
            },
            .{
                .x = @as(f32, @floatFromInt(start_line)),
                .y = 0,
                .w = @as(f32, @floatFromInt(DisplayBase.i2c_width - start_line)),
                .h = @as(f32, @floatFromInt(DisplayBase.i2c_height)),
            },
            x_scale,
        ...
        if (start_line > 0) {
            gk.gfx.draw.texScaleXYRegionAngle(
                ...
                .{
                    .x = @as(f32, @floatFromInt(DisplayBase.padding + DisplayBase.border_width)) + @as(f32, @floatFromInt(DisplayBase.i2c_width)) * display_scale,
                    .y = @as(f32, @floatFromInt(DisplayBase.padding + DisplayBase.border_width + DisplayBase.top_area)) + (shift + @as(f32, @floatFromInt(DisplayBase.i2c_width - start_line)) * x_factor) * display_scale,
                },
                .{
                    .x = 0,
                    .y = 0,
                    .w = @as(f32, @floatFromInt(start_line)),
                    .h = @as(f32, @floatFromInt(DisplayBase.i2c_height)),
                },
                x_scale,
                DisplayBase.display_scale,
